...
namespace Rokid.UXR.Interaction
{
    ...
    public class HeadHandDriver : MonoBehaviour
    {
        ...
        [SerializeField]
        private List<IHeadHandDriver> drivedObjs = new List<IHeadHandDriver>();
        ...
        {
            for (int i = 0; i < drivedObjs.Count; i++)
            {
                drivedObjs[i].OnBeforeChangeHoldHandType(holdHandType);
            }
            for (int i = 0; i < drivedObjs.Count; i++)
            {
                drivedObjs[i].OnChangeHoldHandType(holdHandType);
            }
        ...
        private void OnHandPress()
        {
            for (int i = 0; i < drivedObjs.Count; i++)
            {
                drivedObjs[i].OnHandPress(holdHandType);
            }
        }
