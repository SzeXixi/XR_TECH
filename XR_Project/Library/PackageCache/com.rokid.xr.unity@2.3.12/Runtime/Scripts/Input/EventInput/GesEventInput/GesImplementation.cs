    ...
    public class GesImplementation : MonoBehaviour
    {
        ...
        private void OnTrackedFailed(HandType hand)
        {
            if (hand == HandType.LeftHand)
            {
                ...
                {
                    RKLog.KeyInfo($"====GesImplemetation==== OnTrackedFailed To Process LeftHand Data:{preLeftGesData.ThreeGesKeyInfo()}");
            ...
            else if (hand == HandType.RightHand)
            {
                ...
                {
                    RKLog.KeyInfo($"====GesImplemetation==== OnTrackedFailed To Process RightHand Data:{preRightGesData.ThreeGesKeyInfo()}");
        ...
        {
            if (initGesCache && initGesModule == false && Utils.IsAndroidPlatfrom())
            {
                initGesModule = true;
        ...
        private int CalHandOrientation(Vector3 handFoward)
        {
            float result = Vector3.Dot(handFoward, NativeInterface.NativeAPI.GetHeadPose(out long ts).rotation * Vector3.forward);
        ...
        public Vector3[] GetVertices(float[] data, bool right, Pose cameraPose, float scale = 1.0f)
        {
            Vector3[] vertices;
        ...
        public float GetPinchDistance(HandType type)
        {
            switch (type)
            {
                case HandType.LeftHand:
        ...
        /// <returns></returns>
        public bool HasGesType(GestureType targetGes, int preFrame, HandType hand)
        {
        ...
        /// <param name="preFrame"></param>
        /// <returns></returns>
        public bool HasGesTypeAndHandOrientation(GestureType targetGes, HandOrientation targetHandOri, int preFrame, HandType hand)
        {
            if (hand == HandType.None)
                return false;
            GestureType[] gestureTypeCache = hand == HandType.LeftHand ? leftGestureTypeCache : rightGestureTypeCache;
        ...
        /// <param name="preFrame"></param>
        /// <returns></returns>
        public bool HasHandOrientationType(HandOrientation targetOrientation, int preFrame, HandType hand)
        {
            if (hand == HandType.None)
                return false;
            HandOrientation[] handOrientationCache = hand == HandType.LeftHand ? leftHandOrientationCache : rightHandOrientationCache;
